...
A newly launched exchange --- referred to throughout this report as exchange
...
capital $K$ split evenly between EUR and USD and an asymmetric fee grid
(taker/maker of $0.04\%/0.01\%$ on $A$, $0.02\%/0.009\%$ on $B$ and
$0.03\%/0.009\%$ on $C$). A delta limit caps the EUR inventory we are willing
...
\autoref{sec:book-updates} covers submission and cancellation handling;
\autoref{sec:simulation-loop} documents the simulation loop that binds the
modules together; \autoref{sec:pnl} details the inventory and P\&L
bookkeeping; \autoref{sec:backtest} gathers the Phase 1 backtest outputs;
